...
"@ 
    ...
    If ($retval.IsOnline -eq "ONLINE") {
        Write-Host "Command [Online-Database] [$DatabaseName] completed successfully."
    } else {
        Write-Host "Command [Online-Database] [$DatabaseName] completed unsuccessfully."
    }
...
"@ 
    Invoke-Sqlcmd $SQLCMD -QueryTimeout 3600 -ServerInstance .
    $retval = Invoke-Sqlcmd -Query "select state_desc as IsOnline from sys.databases where name = '$DatabaseName';"
    If ($retval.IsOnline -eq "OFFLINE") {
        Write-Host "Command [Offline-Database] [$DatabaseName] completed successfully."
    } else {
        Write-Host "Command [Offline-Database] [$DatabaseName] completed unsuccessfully."
    }
}
...
Function Refresh-DatabaseImage {
        param(
        [Parameter(Mandatory)]
        [string]$BaseDirectory,
        [Parameter(Mandatory)]
...
Delete-DatabaseImage -NewDatabaseName $NewDatabaseName -BaseDirectory $BaseDirectory
Create-DatabaseImage -DatabaseName $DatabaseName -NewDatabaseName $NewDatabaseName -BaseDirectory $BaseDirectory
}
